  ...
  const filteredAndSortedItems = useMemo(() => {
    let filtered = items.filter((item) => {
      ...
      const searchMatch = !searchQuery || 
        [item.Item_Name, item.Category, item.Description]
          .some(field => field?.toLowerCase().includes(searchQuery.toLowerCase()));
      
      // Category filter
      const categoryMatch = categoryFilter === "All" || 
        mapToGroup(item.Category) === categoryFilter;
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#9aa7a6] absolute left-3 top-1/2 transform -translate-y-1/2" viewBox="0 0 20 20" fill="currentColor">
        ...
        {filteredAndSortedItems.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-[#64706f]">Try adjusting your search or filter criteria</p>
          </div>
        ) : (
              ...
              return (
                <div key={item._id} className="bg-white rounded-lg shadow-sm border border-[#E6F4F3] overflow-hidden hover:shadow-md transition-shadow duration-200" onClick={() => setSelectedItem(item)}>
                  ...
                  <div className="aspect-square bg-gray-100 relative overflow-hidden">
                    ...
                    <div className={`w-full h-full flex items-center justify-center text-4xl text-gray-400 ${item.image ? 'hidden' : 'flex'}`}>
                      🐾
                    </div>
