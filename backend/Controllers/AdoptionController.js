...
const addAdoptions = async (req, res, next) => {
    try {
        ...
        if (!fullName || !email || !age || !phone || !address || !salary) {
        ...
        if (typeof selectedPets === 'string') {
            try { parsedSelectedPets = JSON.parse(selectedPets); }
            catch { return res.status(400).json({ message: "selectedPets must be valid JSON" }); }
        }
