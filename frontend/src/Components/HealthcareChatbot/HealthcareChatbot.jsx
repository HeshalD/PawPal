...
const HealthcareChatbot = ({ role = 'user', context = '' }) => {
  ...
  return (
    <>
        ...
        <div className="fixed bottom-24 right-6 w-80 h-96 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-50">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.isBot ? 'justify-start' : 'justify-end'}`}>
                <div className={`max-w-xs px-4 py-2 rounded-2xl ${m.isBot ? 'bg-gray-100 text-gray-800' : 'bg-gradient-to-r from-pink-500 to-purple-500 text-white'}`}>
              ...
              <div className="grid grid-cols-1 gap-2">
                {suggestions.map((s, i) => (
                  <button
                    key={i}
                    onClick={() => sendText(s)}
                    className="text-left text-sm px-3 py-2 rounded-xl border border-gray-200 hover:border-pink-400 hover:bg-pink-50 transition-colors"
                  >
                    {s}
                  </button>
                ))}
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex space-x-2">
              ...
              />
              <button onClick={send} className="bg-gradient-to-r from-pink-500 to-purple-500 text-white p-2 rounded-full hover:from-pink-600 hover:to-purple-600 transition-colors">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-6 right-6 bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 z-40"
      >
